...
- `memory_read_segment(source_id, segment_id)` calls `GET /v1/segments/{id}?source_id=...`, validates the returned ownership pair, reloads that exact configured source, and returns ordered source text for the committed source boundary.

Hydration fails closed if the source is unavailable or history no longer exists. A boundary-v1 segment hydrates complete turns between its inclusive user boundaries; boundary-v2 resolves an exact span within a v1 user turn. Remote v1 reads follow `X-Next-Cursor` with `before`. Boundary-v3 hydration requires an `opencode-v2` / `source-v1` registry entry and a matching configured endpoint. It follows native cursor pagination, canonicalizes typed source records, and returns the exact inclusive source-message range only when both endpoints exist in order and every record in the range is complete. It also validates the deterministic native segment ID. Metadata does not expose a source fingerprint, so this is not fingerprint verification. Both legacy boundary versions are rejected for native-v2 sources, rather than fabricating parent links. Reads use one bounded deadline for the complete page sequence and reject duplicate messages or repeated cursors. Remote reads never fall back to the writer's SDK; this plugin's writer remains v1-only.
...
Native hydration returns `{id, type, text}` messages with their native origin types, including `synthetic`, alongside boundary version 3 and source-message endpoints. It does not return invented user-message boundaries or parent links.
